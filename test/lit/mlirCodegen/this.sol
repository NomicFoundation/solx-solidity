// RUN: solc --mlir-action=print-init --mmlir --mlir-print-debuginfo %s | FileCheck %s

contract C {
  function f(address a) public {
    a = address(this);
  }

  function g() public view returns (address payable) {
    return payable(address(this));
  }
}

contract P {
  receive() external payable {}

  function self() public view returns (P) {
    return this;
  }
}

// NOTE: Assertions have been autogenerated by test/updFileCheckTest.py
// CHECK: #Constructor = #sol<FunctionKind Constructor>
// CHECK-NEXT: #Contract = #sol<ContractKind Contract>
// CHECK-NEXT: #Default = #sol<RevertStrings Default>
// CHECK-NEXT: #NonPayable = #sol<StateMutability NonPayable>
// CHECK-NEXT: #Osaka = #sol<EvmVersion Osaka>
// CHECK-NEXT: #View = #sol<StateMutability View>
// CHECK-NEXT: #loc3 = loc({{.*}}:3:13)
// CHECK-NEXT: module attributes {llvm.data_layout = "E-p:256:256-i256:256:256-S256-a:256:256", llvm.target_triple = "evm-unknown-unknown", sol.evm_version = #Osaka, sol.revert_strings = #Default} {
// CHECK-NEXT:   sol.contract @C_28 {
// CHECK-NEXT:     sol.func @C_28() attributes {kind = #Constructor, orig_fn_type = () -> (), state_mutability = #NonPayable} {
// CHECK-NEXT:       sol.return loc(#loc1)
// CHECK-NEXT:     } loc(#loc1)
// CHECK-NEXT:     sol.func @f_13(%arg0: !sol.address loc({{.*}}:3:13)) attributes {id = 13 : i64, orig_fn_type = (!sol.address) -> (), selector = -60272102 : i32, state_mutability = #NonPayable} {
// CHECK-NEXT:       %0 = sol.alloca : !sol.ptr<!sol.address, Stack> loc(#loc3)
// CHECK-NEXT:       sol.store %arg0, %0 : !sol.address, !sol.ptr<!sol.address, Stack> loc(#loc3)
// CHECK-NEXT:       %1 = sol.this : !sol.contract<"C_28"> loc(#loc4)
// CHECK-NEXT:       %2 = sol.address_cast %1 : !sol.contract<"C_28"> to !sol.address loc(#loc4)
// CHECK-NEXT:       sol.store %2, %0 : !sol.address, !sol.ptr<!sol.address, Stack> loc(#loc5)
// CHECK-NEXT:       sol.return loc(#loc2)
// CHECK-NEXT:     } loc(#loc2)
// CHECK-NEXT:     sol.func @g_27() -> !sol.address<payable> attributes {id = 27 : i64, orig_fn_type = () -> !sol.address<payable>, selector = -501769330 : i32, state_mutability = #View} {
// CHECK-NEXT:       %0 = sol.alloca : !sol.ptr<!sol.address<payable>, Stack> loc(#loc7)
// CHECK-NEXT:       %c0_ui160 = sol.constant 0 : ui160 loc(#loc7)
// CHECK-NEXT:       %1 = sol.address_cast %c0_ui160 : ui160 to !sol.address<payable> loc(#loc7)
// CHECK-NEXT:       sol.store %1, %0 : !sol.address<payable>, !sol.ptr<!sol.address<payable>, Stack> loc(#loc7)
// CHECK-NEXT:       %2 = sol.this : !sol.contract<"C_28"> loc(#loc8)
// CHECK-NEXT:       %3 = sol.address_cast %2 : !sol.contract<"C_28"> to !sol.address loc(#loc8)
// CHECK-NEXT:       %4 = sol.address_cast %3 : !sol.address to !sol.address<payable> loc(#loc8)
// CHECK-NEXT:       sol.return %4 : !sol.address<payable> loc(#loc9)
// CHECK-NEXT:     } loc(#loc6)
// CHECK-NEXT:   } {kind = #Contract} loc(#loc1)
// CHECK-NEXT: } loc(#loc)
// CHECK-NEXT: #loc = loc(unknown)
// CHECK-NEXT: #loc1 = loc({{.*}}:2:0)
// CHECK-NEXT: #loc2 = loc({{.*}}:3:2)
// CHECK-NEXT: #loc4 = loc({{.*}}:4:16)
// CHECK-NEXT: #loc5 = loc({{.*}}:4:4)
// CHECK-NEXT: #loc6 = loc({{.*}}:7:2)
// CHECK-NEXT: #loc7 = loc({{.*}}:7:36)
// CHECK-NEXT: #loc8 = loc({{.*}}:8:27)
// CHECK-NEXT: #loc9 = loc({{.*}}:8:4)
// CHECK-NEXT: #Constructor = #sol<FunctionKind Constructor>
// CHECK-NEXT: #Contract = #sol<ContractKind Contract>
// CHECK-NEXT: #Default = #sol<RevertStrings Default>
// CHECK-NEXT: #NonPayable = #sol<StateMutability NonPayable>
// CHECK-NEXT: #Osaka = #sol<EvmVersion Osaka>
// CHECK-NEXT: #Payable = #sol<StateMutability Payable>
// CHECK-NEXT: #Receive = #sol<FunctionKind Receive>
// CHECK-NEXT: #View = #sol<StateMutability View>
// CHECK-NEXT: module attributes {llvm.data_layout = "E-p:256:256-i256:256:256-S256-a:256:256", llvm.target_triple = "evm-unknown-unknown", sol.evm_version = #Osaka, sol.revert_strings = #Default} {
// CHECK-NEXT:   sol.contract @P_42 {
// CHECK-NEXT:     sol.func @P_42() attributes {kind = #Constructor, orig_fn_type = () -> (), state_mutability = #NonPayable} {
// CHECK-NEXT:       sol.return loc(#loc1)
// CHECK-NEXT:     } loc(#loc1)
// CHECK-NEXT:     sol.func @_32() attributes {id = 32 : i64, kind = #Receive, state_mutability = #Payable} {
// CHECK-NEXT:       sol.return loc(#loc2)
// CHECK-NEXT:     } loc(#loc2)
// CHECK-NEXT:     sol.func @self_41() -> !sol.contract<"P_42", payable> attributes {id = 41 : i64, orig_fn_type = () -> !sol.contract<"P_42", payable>, selector = 1896144306 : i32, state_mutability = #View} {
// CHECK-NEXT:       %0 = sol.alloca : !sol.ptr<!sol.contract<"P_42", payable>, Stack> loc(#loc4)
// CHECK-NEXT:       %c0_ui160 = sol.constant 0 : ui160 loc(#loc4)
// CHECK-NEXT:       %1 = sol.address_cast %c0_ui160 : ui160 to !sol.address<payable> loc(#loc4)
// CHECK-NEXT:       %2 = sol.address_cast %1 : !sol.address<payable> to !sol.contract<"P_42", payable> loc(#loc4)
// CHECK-NEXT:       sol.store %2, %0 : !sol.contract<"P_42", payable>, !sol.ptr<!sol.contract<"P_42", payable>, Stack> loc(#loc4)
// CHECK-NEXT:       %3 = sol.this : !sol.contract<"P_42", payable> loc(#loc5)
// CHECK-NEXT:       sol.return %3 : !sol.contract<"P_42", payable> loc(#loc6)
// CHECK-NEXT:     } loc(#loc3)
// CHECK-NEXT:   } {kind = #Contract} loc(#loc1)
// CHECK-NEXT: } loc(#loc)
// CHECK-NEXT: #loc = loc(unknown)
// CHECK-NEXT: #loc1 = loc({{.*}}:12:0)
// CHECK-NEXT: #loc2 = loc({{.*}}:13:2)
// CHECK-NEXT: #loc3 = loc({{.*}}:15:2)
// CHECK-NEXT: #loc4 = loc({{.*}}:15:39)
// CHECK-NEXT: #loc5 = loc({{.*}}:16:11)
// CHECK-NEXT: #loc6 = loc({{.*}}:16:4)
// CHECK-EMPTY:
