// RUN: solc --mlir-action=print-init --mmlir --mlir-print-debuginfo %s | FileCheck %s

function f(bool a) returns (uint) {
  if (a)
    return 2;
  return 0;
}

function g(bool a) {
  if (a)
    return;
  return;
}

function h(bool a) {
  if (a)
    return;
}

// NOTE: Assertions have been autogenerated by test/updFileCheckTest.py
// CHECK: #Default = #sol<RevertStrings Default>
// CHECK-NEXT: #NonPayable = #sol<StateMutability NonPayable>
// CHECK-NEXT: #Osaka = #sol<EvmVersion Osaka>
// CHECK-NEXT: #loc2 = loc({{.*}}:2:11)
// CHECK-NEXT: #loc11 = loc({{.*}}:8:11)
// CHECK-NEXT: #loc17 = loc({{.*}}:14:11)
// CHECK-NEXT: module attributes {llvm.data_layout = "E-p:256:256-i256:256:256-S256-a:256:256", llvm.target_triple = "evm-unknown-unknown", sol.evm_version = #Osaka, sol.revert_strings = #Default} {
// CHECK-NEXT:   sol.func @f_14(%arg0: i1 loc({{.*}}:2:11)) -> ui256 attributes {id = 14 : i64, state_mutability = #NonPayable} {
// CHECK-NEXT:     %0 = sol.alloca : !sol.ptr<i1, Stack> loc(#loc2)
// CHECK-NEXT:     sol.store %arg0, %0 : i1, !sol.ptr<i1, Stack> loc(#loc2)
// CHECK-NEXT:     %1 = sol.alloca : !sol.ptr<ui256, Stack> loc(#loc3)
// CHECK-NEXT:     %c0_ui256 = sol.constant 0 : ui256 loc(#loc3)
// CHECK-NEXT:     sol.store %c0_ui256, %1 : ui256, !sol.ptr<ui256, Stack> loc(#loc3)
// CHECK-NEXT:     %2 = sol.load %0 : !sol.ptr<i1, Stack>, i1 loc(#loc4)
// CHECK-NEXT:     sol.if %2 {
// CHECK-NEXT:       %c2_ui8 = sol.constant 2 : ui8 loc(#loc6)
// CHECK-NEXT:       %4 = sol.cast %c2_ui8 : ui8 to ui256 loc(#loc6)
// CHECK-NEXT:       sol.return %4 : ui256 loc(#loc7)
// CHECK-NEXT:     ^bb1:  // no predecessors
// CHECK-NEXT:       sol.yield loc(#loc5)
// CHECK-NEXT:     } else {
// CHECK-NEXT:     } loc(#loc5)
// CHECK-NEXT:     %c0_ui8 = sol.constant 0 : ui8 loc(#loc8)
// CHECK-NEXT:     %3 = sol.cast %c0_ui8 : ui8 to ui256 loc(#loc8)
// CHECK-NEXT:     sol.return %3 : ui256 loc(#loc9)
// CHECK-NEXT:   } loc(#loc1)
// CHECK-NEXT:   sol.func @g_24(%arg0: i1 loc({{.*}}:8:11)) attributes {id = 24 : i64, state_mutability = #NonPayable} {
// CHECK-NEXT:     %0 = sol.alloca : !sol.ptr<i1, Stack> loc(#loc11)
// CHECK-NEXT:     sol.store %arg0, %0 : i1, !sol.ptr<i1, Stack> loc(#loc11)
// CHECK-NEXT:     %1 = sol.load %0 : !sol.ptr<i1, Stack>, i1 loc(#loc12)
// CHECK-NEXT:     sol.if %1 {
// CHECK-NEXT:       sol.return loc(#loc14)
// CHECK-NEXT:     ^bb1:  // no predecessors
// CHECK-NEXT:       sol.yield loc(#loc13)
// CHECK-NEXT:     } else {
// CHECK-NEXT:     } loc(#loc13)
// CHECK-NEXT:     sol.return loc(#loc15)
// CHECK-NEXT:   } loc(#loc10)
// CHECK-NEXT:   sol.func @h_33(%arg0: i1 loc({{.*}}:14:11)) attributes {id = 33 : i64, state_mutability = #NonPayable} {
// CHECK-NEXT:     %0 = sol.alloca : !sol.ptr<i1, Stack> loc(#loc17)
// CHECK-NEXT:     sol.store %arg0, %0 : i1, !sol.ptr<i1, Stack> loc(#loc17)
// CHECK-NEXT:     %1 = sol.load %0 : !sol.ptr<i1, Stack>, i1 loc(#loc18)
// CHECK-NEXT:     sol.if %1 {
// CHECK-NEXT:       sol.return loc(#loc20)
// CHECK-NEXT:     ^bb1:  // no predecessors
// CHECK-NEXT:       sol.yield loc(#loc19)
// CHECK-NEXT:     } else {
// CHECK-NEXT:     } loc(#loc19)
// CHECK-NEXT:     sol.return loc(#loc16)
// CHECK-NEXT:   } loc(#loc16)
// CHECK-NEXT: } loc(#loc)
// CHECK-NEXT: #loc = loc(unknown)
// CHECK-NEXT: #loc1 = loc({{.*}}:2:0)
// CHECK-NEXT: #loc3 = loc({{.*}}:2:28)
// CHECK-NEXT: #loc4 = loc({{.*}}:3:6)
// CHECK-NEXT: #loc5 = loc({{.*}}:3:2)
// CHECK-NEXT: #loc6 = loc({{.*}}:4:11)
// CHECK-NEXT: #loc7 = loc({{.*}}:4:4)
// CHECK-NEXT: #loc8 = loc({{.*}}:5:9)
// CHECK-NEXT: #loc9 = loc({{.*}}:5:2)
// CHECK-NEXT: #loc10 = loc({{.*}}:8:0)
// CHECK-NEXT: #loc12 = loc({{.*}}:9:6)
// CHECK-NEXT: #loc13 = loc({{.*}}:9:2)
// CHECK-NEXT: #loc14 = loc({{.*}}:10:4)
// CHECK-NEXT: #loc15 = loc({{.*}}:11:2)
// CHECK-NEXT: #loc16 = loc({{.*}}:14:0)
// CHECK-NEXT: #loc18 = loc({{.*}}:15:6)
// CHECK-NEXT: #loc19 = loc({{.*}}:15:2)
// CHECK-NEXT: #loc20 = loc({{.*}}:16:4)
// CHECK-EMPTY:
