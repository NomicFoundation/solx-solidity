// RUN: solc --strict-assembly --mlir-action=print-init --mmlir --mlir-print-debuginfo %s | FileCheck %s

object "Test" {
  code {
    let a := mload(64)
    return(a, 0)
  }
  object "Test_deployed" {
    code {
      let a := mload(64)
      return(a, 0)
    }
  }
}

// NOTE: Assertions have been autogenerated by test/updFileCheckTest.py
// CHECK: #Osaka = #sol<EvmVersion Osaka>
// CHECK-NEXT: module attributes {llvm.data_layout = "E-p:256:256-i256:256:256-S256-a:256:256", llvm.target_triple = "evm-unknown-unknown", sol.evm_version = #Osaka} {
// CHECK-NEXT:   yul.object @Test {
// CHECK-NEXT:     %c64_i256 = yul.constant 64 loc(#loc1)
// CHECK-NEXT:     %0 = yul.mload %c64_i256 loc(#loc2)
// CHECK-NEXT:     %1 = yul.alloca : !yul.ptr loc(#loc3)
// CHECK-NEXT:     yul.store %0, %1 : i256, !yul.ptr loc(#loc4)
// CHECK-NEXT:     %2 = yul.load %1 : !yul.ptr -> i256 loc(#loc5)
// CHECK-NEXT:     %c0_i256 = yul.constant 0 loc(#loc6)
// CHECK-NEXT:     yul.return %2, %c0_i256 loc(#loc7)
// CHECK-NEXT:     yul.object @Test_deployed {
// CHECK-NEXT:       %c64_i256_0 = yul.constant 64 loc(#loc8)
// CHECK-NEXT:       %3 = yul.mload %c64_i256_0 loc(#loc9)
// CHECK-NEXT:       %4 = yul.alloca : !yul.ptr loc(#loc10)
// CHECK-NEXT:       yul.store %3, %4 : i256, !yul.ptr loc(#loc11)
// CHECK-NEXT:       %5 = yul.load %4 : !yul.ptr -> i256 loc(#loc12)
// CHECK-NEXT:       %c0_i256_1 = yul.constant 0 loc(#loc13)
// CHECK-NEXT:       yul.return %5, %c0_i256_1 loc(#loc14)
// CHECK-NEXT:     } loc(#loc)
// CHECK-NEXT:   } loc(#loc)
// CHECK-NEXT: } loc(#loc)
// CHECK-NEXT: #loc = loc(unknown)
// CHECK-NEXT: #loc1 = loc({{.*}}:4:19)
// CHECK-NEXT: #loc2 = loc({{.*}}:4:13)
// CHECK-NEXT: #loc3 = loc({{.*}}:4:8)
// CHECK-NEXT: #loc4 = loc({{.*}}:4:4)
// CHECK-NEXT: #loc5 = loc({{.*}}:5:11)
// CHECK-NEXT: #loc6 = loc({{.*}}:5:14)
// CHECK-NEXT: #loc7 = loc({{.*}}:5:4)
// CHECK-NEXT: #loc8 = loc({{.*}}:9:21)
// CHECK-NEXT: #loc9 = loc({{.*}}:9:15)
// CHECK-NEXT: #loc10 = loc({{.*}}:9:10)
// CHECK-NEXT: #loc11 = loc({{.*}}:9:6)
// CHECK-NEXT: #loc12 = loc({{.*}}:10:13)
// CHECK-NEXT: #loc13 = loc({{.*}}:10:16)
// CHECK-NEXT: #loc14 = loc({{.*}}:10:6)
// CHECK-EMPTY:
