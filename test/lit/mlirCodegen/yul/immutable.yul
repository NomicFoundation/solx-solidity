// RUN: solc --strict-assembly --mlir-action=print-init --mmlir --mlir-print-debuginfo %s | FileCheck %s

object "Test" {
  code {
    setimmutable(10, "foo", 0x20)
    setimmutable(0, "10000000000000000000000000", 1)
  }
  object "Test_deployed" {
    code {
      let a := loadimmutable("foo")
      let b := loadimmutable("10000000000000000000000000")
    }
  }
}
// NOTE: Assertions have been autogenerated by test/updFileCheckTest.py
// CHECK: #Osaka = #sol<EvmVersion Osaka>
// CHECK-NEXT: module attributes {llvm.data_layout = "E-p:256:256-i256:256:256-S256-a:256:256", llvm.target_triple = "evm-unknown-unknown", sol.evm_version = #Osaka} {
// CHECK-NEXT:   yul.object @Test {
// CHECK-NEXT:     %c10_i256 = yul.constant 10 : i256 : i256 loc(#loc1)
// CHECK-NEXT:     %c32_i256 = yul.constant 32 : i256 : i256 loc(#loc2)
// CHECK-NEXT:     yul.setimmutable %c10_i256, "foo", %c32_i256 loc(#loc3)
// CHECK-NEXT:     %c0_i256 = yul.constant 0 : i256 : i256 loc(#loc4)
// CHECK-NEXT:     %c1_i256 = yul.constant 1 : i256 : i256 loc(#loc5)
// CHECK-NEXT:     yul.setimmutable %c0_i256, "10000000000000000000000000", %c1_i256 loc(#loc6)
// CHECK-NEXT:     yul.object @Test_deployed {
// CHECK-NEXT:       %0 = yul.loadimmutable "foo" loc(#loc7)
// CHECK-NEXT:       %c1_i256_0 = yul.constant 1 : i256 : i256 loc(#loc8)
// CHECK-NEXT:       %1 = llvm.alloca %c1_i256_0 x i256 {alignment = 32 : i64} : (i256) -> !llvm.ptr loc(#loc9)
// CHECK-NEXT:       llvm.store %0, %1 {alignment = 32 : i64} : i256, !llvm.ptr loc(#loc8)
// CHECK-NEXT:       %2 = yul.loadimmutable "10000000000000000000000000" loc(#loc10)
// CHECK-NEXT:       %c1_i256_1 = yul.constant 1 : i256 : i256 loc(#loc11)
// CHECK-NEXT:       %3 = llvm.alloca %c1_i256_1 x i256 {alignment = 32 : i64} : (i256) -> !llvm.ptr loc(#loc12)
// CHECK-NEXT:       llvm.store %2, %3 {alignment = 32 : i64} : i256, !llvm.ptr loc(#loc11)
// CHECK-NEXT:     } loc(#loc)
// CHECK-NEXT:   } loc(#loc)
// CHECK-NEXT: } loc(#loc)
// CHECK-NEXT: #loc = loc(unknown)
// CHECK-NEXT: #loc1 = loc({{.*}}:4:17)
// CHECK-NEXT: #loc2 = loc({{.*}}:4:28)
// CHECK-NEXT: #loc3 = loc({{.*}}:4:4)
// CHECK-NEXT: #loc4 = loc({{.*}}:5:17)
// CHECK-NEXT: #loc5 = loc({{.*}}:5:50)
// CHECK-NEXT: #loc6 = loc({{.*}}:5:4)
// CHECK-NEXT: #loc7 = loc({{.*}}:9:15)
// CHECK-NEXT: #loc8 = loc({{.*}}:9:6)
// CHECK-NEXT: #loc9 = loc({{.*}}:9:10)
// CHECK-NEXT: #loc10 = loc({{.*}}:10:15)
// CHECK-NEXT: #loc11 = loc({{.*}}:10:6)
// CHECK-NEXT: #loc12 = loc({{.*}}:10:10)
// CHECK-EMPTY:
