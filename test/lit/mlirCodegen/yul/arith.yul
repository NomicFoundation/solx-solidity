// RUN: solc --strict-assembly --mlir-action=print-init --mmlir --mlir-print-debuginfo %s | FileCheck %s

object "Test" {
  code {
    let a := add(0, 1)
    let b := sub(0, 1)
    let c := shr(0, 1)
  }
}

// NOTE: Assertions have been autogenerated by test/updFileCheckTest.py
// CHECK: module {
// CHECK-NEXT:   sol.object @Test {
// CHECK-NEXT:     %c1_i256 = arith.constant 1 : i256 loc(#loc1)
// CHECK-NEXT:     %0 = llvm.alloca %c1_i256 x i256 {alignment = 32 : i64} : (i256) -> !llvm.ptr<i256> loc(#loc2)
// CHECK-NEXT:     %c0_i256 = arith.constant 0 : i256 loc(#loc3)
// CHECK-NEXT:     %c1_i256_0 = arith.constant 1 : i256 loc(#loc4)
// CHECK-NEXT:     %1 = arith.addi %c0_i256, %c1_i256_0 : i256 loc(#loc5)
// CHECK-NEXT:     llvm.store %1, %0 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc1)
// CHECK-NEXT:     %c1_i256_1 = arith.constant 1 : i256 loc(#loc6)
// CHECK-NEXT:     %2 = llvm.alloca %c1_i256_1 x i256 {alignment = 32 : i64} : (i256) -> !llvm.ptr<i256> loc(#loc7)
// CHECK-NEXT:     %c0_i256_2 = arith.constant 0 : i256 loc(#loc8)
// CHECK-NEXT:     %c1_i256_3 = arith.constant 1 : i256 loc(#loc9)
// CHECK-NEXT:     %3 = arith.subi %c0_i256_2, %c1_i256_3 : i256 loc(#loc10)
// CHECK-NEXT:     llvm.store %3, %2 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc6)
// CHECK-NEXT:     %c1_i256_4 = arith.constant 1 : i256 loc(#loc11)
// CHECK-NEXT:     %4 = llvm.alloca %c1_i256_4 x i256 {alignment = 32 : i64} : (i256) -> !llvm.ptr<i256> loc(#loc12)
// CHECK-NEXT:     %c1_i256_5 = arith.constant 1 : i256 loc(#loc13)
// CHECK-NEXT:     %c0_i256_6 = arith.constant 0 : i256 loc(#loc14)
// CHECK-NEXT:     %5 = arith.shrui %c1_i256_5, %c0_i256_6 : i256 loc(#loc15)
// CHECK-NEXT:     llvm.store %5, %4 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc11)
// CHECK-NEXT:   } loc(#loc)
// CHECK-NEXT: } loc(#loc)
// CHECK-NEXT: #loc = loc(unknown)
// CHECK-NEXT: #loc1 = loc({{.*}}:4:4)
// CHECK-NEXT: #loc2 = loc({{.*}}:4:8)
// CHECK-NEXT: #loc3 = loc({{.*}}:4:17)
// CHECK-NEXT: #loc4 = loc({{.*}}:4:20)
// CHECK-NEXT: #loc5 = loc({{.*}}:4:13)
// CHECK-NEXT: #loc6 = loc({{.*}}:5:4)
// CHECK-NEXT: #loc7 = loc({{.*}}:5:8)
// CHECK-NEXT: #loc8 = loc({{.*}}:5:17)
// CHECK-NEXT: #loc9 = loc({{.*}}:5:20)
// CHECK-NEXT: #loc10 = loc({{.*}}:5:13)
// CHECK-NEXT: #loc11 = loc({{.*}}:6:4)
// CHECK-NEXT: #loc12 = loc({{.*}}:6:8)
// CHECK-NEXT: #loc13 = loc({{.*}}:6:20)
// CHECK-NEXT: #loc14 = loc({{.*}}:6:17)
// CHECK-NEXT: #loc15 = loc({{.*}}:6:13)
// CHECK-EMPTY:
