...
struct ConcatOpLowering : public OpConversionPattern<sol::ConcatOp> {
  ...
  LogicalResult matchAndRewrite(sol::ConcatOp op, OpAdaptor adaptor,
    ...
    Value dataEnd = dataStart;
    for (auto [origDataAddr, dataAddr] :
         llvm::zip(op.getArgs(), adaptor.getArgs())) {
      Type ty = origDataAddr.getType();
      sol::DataLocation dataLoc = sol::getDataLocation(ty);
      Value lengthSlot = evmB.genLoad(dataAddr, dataLoc, loc);
      Value length = evmB.genStringSize(lengthSlot, dataLoc, loc);
      evmB.genCopyStringDataToMemory(dataAddr, lengthSlot, length, dataEnd,
                                     dataLoc, loc);
      dataEnd = r.create<arith::AddIOp>(loc, dataEnd, length);
    }
