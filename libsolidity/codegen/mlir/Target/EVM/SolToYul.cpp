...
static Value genYulModOp(ConversionPatternRewriter &r, Location loc, Value x,
  ...
  // Yul mod ops work with i256.
  Value x256 = bExt.genIntCast(256, false, x);
  Value y256 = bExt.genIntCast(256, false, y);
  Value mod256 = bExt.genIntCast(256, false, mod);
...
static Value getCryptoHashLowering(OpT op, typename OpT::Adaptor adaptor,
  ...
  Value dstAddr = evmB.genFreePtr();
  r.create<yul::MCopyOp>(loc, dstAddr, dataSlot, dataLen);
  // Hashing functions store their result in scratch space (0x00–0x3f).
  mlir::Value status =
      r.create<yul::StaticCallOp>(loc, gas, adaptor.getPreCompileAddr(),
                                  /*inpOffset=*/dstAddr, dataLen,
  ...
  uint32_t resShiftAmt = op.getResShiftAmt();
  if (resShiftAmt)
    res = r.create<yul::ShlOp>(loc, bExt.genI256Const(resShiftAmt), res);
...
struct Sha256OpLowering : public OpConversionPattern<sol::Sha256Op> {
  ...
  LogicalResult matchAndRewrite(sol::Sha256Op op, OpAdaptor adaptor,
                                ConversionPatternRewriter &r) const override {
    r.replaceOp(op, getCryptoHashLowering(op, adaptor, r));
...
struct EcrecoverOpLowering : public OpConversionPattern<sol::EcrecoverOp> {
  ...
  LogicalResult matchAndRewrite(sol::EcrecoverOp op, OpAdaptor adaptor,
    ...
    // Store hash
    r.create<yul::MStoreOp>(loc, dstAddr, adaptor.getHash());

    // Storev
    Value vZext =
        r.create<arith::ExtUIOp>(loc, r.getIntegerType(256), adaptor.getV());
    Value v8 = r.create<arith::AndIOp>(loc, vZext, bExt.genI256Const(0xFF));
    Value curDst = r.create<arith::AddIOp>(loc, dstAddr, bExt.genI256Const(32));
    r.create<yul::MStoreOp>(loc, curDst, v8);

    // Store r
    curDst = r.create<arith::AddIOp>(loc, dstAddr, bExt.genI256Const(64));
    r.create<yul::MStoreOp>(loc, curDst, adaptor.getR());

    // Store s
    curDst = r.create<arith::AddIOp>(loc, dstAddr, bExt.genI256Const(96));
    r.create<yul::MStoreOp>(loc, curDst, adaptor.getS());
    ...
    // Hashing functions store their result in scratch space (0x00–0x3f).
    Value gas = r.create<mlir::yul::GasOp>(loc);
    mlir::Value status = r.create<yul::StaticCallOp>(
        loc, gas, adaptor.getPreCompileAddr(),
        /*inpOffset=*/dstAddr, bExt.genI256Const(128),
        /*outOffset=*/zero, /*outSize=*/retSize);
